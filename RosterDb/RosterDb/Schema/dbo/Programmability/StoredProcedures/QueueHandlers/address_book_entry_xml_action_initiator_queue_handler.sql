...
/*
	The procedure implements [<schema>].[ADDRESS_BOOK_ENTRY_XML_ACTION_INITIATOR] queue entry handler
*/
CREATE PROCEDURE [dbo].[address_book_entry_xml_action_initiator_queue_handler]
	...
	BEGIN TRY
		...
		BEGIN
			...
			(
				...
				FROM
					[dbo].[ADDRESS_BOOK_ENTRY_XML_ACTION_INITIATOR]
			), TIMEOUT 600;
			...
			IF @message_type = N'//CompanyName/Roster/AddressBookEntryXmlAction/Parse/Reply'
			BEGIN
				
				EXEC [dbo].[companyname_roster_addressbookentryxmlaction_parse_reply_message_handler] 
					@message_body_xml = @message_body_xml
					, @reply_message_body_xml = @reply_message_body_xml OUT

				-- reply on complete execution
				--;SEND 
			...
			-- end conversation
			END CONVERSATION @init_dlg_handle WITH CLEANUP;
