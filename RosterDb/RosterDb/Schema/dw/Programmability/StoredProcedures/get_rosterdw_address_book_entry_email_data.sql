...
*/
CREATE PROCEDURE [dw].[get_rosterdw_address_book_entry_email_data]
...
BEGIN
	...
	BEGIN TRY
		...
		IF @tran_count = 0
			...
			-- get address book ids to be processed
			UPDATE [dw].[ROSTERDW_ADDRESS_BOOK_TRANSFER_CANDIDATE] WITH(READPAST)
				SET [Entries_are_transfered] = 1
			OUTPUT inserted.[Address_book_id] INTO #ADDRESS_BOOK_ID_2B_PROCESSED([Address_book_id])
			...
			FROM
				...
				INNER JOIN [dbo].[ADDRESS_BOOK_ENTRY] abe ON abe.[Address_book_id] = t.[Address_book_id] 
