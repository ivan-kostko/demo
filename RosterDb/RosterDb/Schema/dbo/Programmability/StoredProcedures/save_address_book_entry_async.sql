...
/*
	The procedure implements the entrance point interface to queue up a message to save entries for address book
*/

CREATE PROCEDURE [dbo].[save_address_book_entry_async]
	@address_book_id										INT			-- current active users address book
	...
	, @address_book_entry_xml_temp_storage_id				INT			-- identificator at [<schema>].[ADDRESS_BOOK_ENTRY_XML_TEMP_STORAGE].[Id] if it is used
AS
BEGIN
	...
	BEGIN TRY
		...
		IF @tran_count = 0
			...
			EXEC [dbo].[companyname_roster_addressbookentryaction_saveonaddressbookcreated_request_message_sender]
