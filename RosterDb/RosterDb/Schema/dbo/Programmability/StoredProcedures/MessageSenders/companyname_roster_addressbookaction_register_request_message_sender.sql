...
/*
	The procedure implements message sender for message type = [//CompanyName/Roster/AddressBookAction/Register/Request]
*/
CREATE PROCEDURE [dbo].[companyname_roster_addressbookaction_register_request_message_sender]
	...
	BEGIN TRY
		...
		IF @tran_count = 0
			...
			BEGIN DIALOG @dlg_handle
				FROM SERVICE	[//CompanyName/Roster/Address_Book_Action/Initiator]
				TO	SERVICE		'//CompanyName/Roster/Address_Book_Action/Target'
				ON	CONTRACT	[//CompanyName/Roster/AddressBookActionContract]
				WITH ENCRYPTION=OFF;       
			   
			SEND 
				ON CONVERSATION @dlg_handle 
				MESSAGE TYPE [//CompanyName/Roster/AddressBookAction/Register/Request]
				(@message_body_xml);   
