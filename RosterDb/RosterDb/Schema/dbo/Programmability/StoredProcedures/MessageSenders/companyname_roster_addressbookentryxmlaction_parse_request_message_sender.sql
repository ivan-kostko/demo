...
/*
	The procedure implements message sender for message type = [//CompanyName/Roster/AddressBookEntryXmlAction/Parse/Request]
*/
CREATE PROCEDURE [dbo].[companyname_roster_addressbookentryxmlaction_parse_request_message_sender]
	...
	BEGIN TRY
		...
		IF @tran_count = 0
			...
			BEGIN DIALOG @dlg_handle
				FROM SERVICE	[//CompanyName/Roster/Address_Book_Entry_Xml_Action/Initiator]
				TO	SERVICE		'//CompanyName/Roster/Address_Book_Entry_Xml_Action/Target'
				ON	CONTRACT	[//CompanyName/Roster/AddressBookEntryXmlActionContract]
				WITH ENCRYPTION=OFF;       
			   
			SEND 
				ON CONVERSATION @dlg_handle 
				MESSAGE TYPE [//CompanyName/Roster/AddressBookEntryXmlAction/Parse/Request]
				(@message_body_xml);   
