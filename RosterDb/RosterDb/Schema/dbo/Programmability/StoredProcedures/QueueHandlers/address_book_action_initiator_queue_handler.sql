...
/*
	The procedure implements [<schema>].[ADDRESS_BOOK_ACTION_INITIATOR] queue entry handler
*/
CREATE PROCEDURE [dbo].[address_book_action_initiator_queue_handler]
	...
	BEGIN TRY
		...
		BEGIN
			...
			(
				...
				FROM
					[dbo].[ADDRESS_BOOK_ACTION_INITIATOR]
			), TIMEOUT 600;
			...
			IF @message_type = N'//CompanyName/Roster/AddressBookAction/Register/Reply'
			BEGIN
				
				EXEC [dbo].[companyname_roster_addressbookaction_register_reply_message_handler] 
